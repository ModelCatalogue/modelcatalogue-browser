#angular.module('mc.core.ui.catalogueElementViewReadOnly', ['mc.core.catalogueElementEnhancer', 'mc.core.listReferenceEnhancer', 'mc.core.listEnhancer', 'mc.util.names', 'mc.util.messages', 'mc.core.ui.columns', 'ui.router']).directive 'catalogueElementViewReadOnly',  [-> {
#restrict: 'E'
#replace: true
#scope:
#  element: '='
...
#
#controller: ['$scope', '$log', '$filter', '$q', '$state', 'enhance', 'names', 'columns', 'messages', '$rootScope', 'catalogueElementResource', 'modelCatalogueApiRoot','ReadonlyFilter', ($scope, $log, $filter, $q, $state, enhance, names, columns, messages, $rootScope, catalogueElementResource,modelCatalogueApiRoot,ReadonlyFilter) ->
...
#      ReadonlyFilter.refineTab(element,tabDefinition)
#
#      if!(ReadonlyFilter.isTabExcluded(element,tabDefinition))
#        tabs.push tabDefinition
#
#
...
angular.module('mc.core.ui.catalogueElementViewReadOnly', ['mc.core.readonlyFilter','mc.core.catalogueElementEnhancer', 'mc.core.listReferenceEnhancer', 'mc.core.listEnhancer', 'mc.util.names', 'mc.util.messages', 'mc.core.ui.columns', 'ui.router']).directive 'catalogueElementViewReadOnly',  [-> {
    restrict: 'E'
    replace: true
    scope:
      element: '='
...
controller: ['$scope', '$log', '$filter', '$q', '$state', 'enhance', 'names', 'columns', 'messages', '$rootScope' ,'modelCatalogueApiRoot', 'ReadonlyFilter',($scope, $log, $filter, $q, $state, enhance, names, columns, messages, $rootScope,modelCatalogueApiRoot,ReadonlyFilter) ->
      ...
      #Should be added in ReadOnly Version
      $scope.reports = null
      if($scope.element.elementType == "org.modelcatalogue.core.Model")
        $scope.reports = [{name: "COSD", url: modelCatalogueApiRoot + "/dataArchitect/getSubModelElements/" + $scope.element.id + "?format=xlsx&report=COSD"}, {name: "NHIC", url: modelCatalogueApiRoot + "/dataArchitect/getSubModelElements/" + $scope.element.id + "?format=xlsx&report=NHIC"}]
      ...
      onElementUpdate = (element) ->
        ...
        for name, fn of element when enhance.isEnhancedBy(fn, 'listReference')
          ...
          #Should be added in ReadOnly Version
#         #Rename tab names and their table columns
          ReadonlyFilter.refineTab(element,tabDefinition)
