angular.module('mc.core.ui.enumeratedDataTypeViewReadOnly', ['mc.core.catalogueElementEnhancer', 'mc.core.listReferenceEnhancer', 'mc.core.listEnhancer', 'mc.util.names', 'mc.util.messages', 'mc.core.ui.columns', 'ui.router']).directive 'enumerateddatatypeViewReadOnly',  [-> {
restrict: 'E'
replace: true
scope:
  element: '='
...
controller: ['$scope', '$log', '$filter', '$q', '$state', 'enhance', 'names', 'columns', 'messages', '$rootScope' , ($scope, $log, $filter, $q, $state, enhance, names, columns, messages, $rootScope) ->
  ...
  onElementUpdate = (element) ->
    ...
    for name, fn of element when enhance.isEnhancedBy(fn, 'listReference')
      ...
      removeHeaders = ["History","Superseded By","Supersedes","Value Domains"]
